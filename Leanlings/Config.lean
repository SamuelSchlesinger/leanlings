...
private def analysisExercises : Array Exercise := #[
  ...
  { name := "r_refl", dir := "Setoid",
    hint := "Unfold `PreRat.r`: the goal is an equation whose two sides are the same expression." },
  { name := "r_symm", dir := "Setoid",
    hint := "Unfold `PreRat.r` in `h` and in the goal. The goal is `h` read from right to left." },
  { name := "r_trans", dir := "Setoid",
    hint := "`apply Int.eq_of_mul_eq_mul_right (a := b.den)` trades the goal for its multiple by `b.den`; you owe `b.den ≠ 0`, which follows from `b.den_pos`.\nThen a `calc` chain, reordering factors with `Int.mul_right_comm` so that `hab` and then `hbc` can be rewritten in." },
  -- WellDef
  { name := "add_resp", dir := "WellDef",
    hint := "`simp only [PreRat.r, PreRat.add] at *` exposes the integer equation in the goal and in both hypotheses. The goal follows from the hypotheses by ring arithmetic." },
  { name := "mul_resp", dir := "WellDef",
    hint := "Same shape as `add_resp`, unfolding `PreRat.mul`." },
  { name := "neg_resp", dir := "WellDef",
    hint := "Same shape as `add_resp`, unfolding `PreRat.neg`." },
  { name := "lt_imp", dir := "WellDef",
    hint := "Scale `h` by the positive product `c.den * d.den`. Two `grind`-proved equations rewrite each side of the scaled inequality into the corresponding side of the goal times `a.den * b.den`; cancel that positive factor with `Int.mul_lt_mul_right`." },
  { name := "le_imp", dir := "WellDef",
    hint := "Mirror `lt_imp`: scale with `Int.mul_le_mul_of_nonneg_right` and cancel with `Int.mul_le_mul_right`." },
  -- Rat
  { name := "add_comm", dir := "Rat",
    hint := "`induction x using MyRat.ind with | _ a b hb =>`, and likewise for `y` with fresh names. Then `add_mk` computes both sums and `mk_eq` gives the integer equation." },
  { name := "add_assoc", dir := "Rat",
    hint := "Three representatives, then `add_mk` until both sides are single fractions. Count the sums: four rewrites." },
  { name := "zero_add", dir := "Rat",
    hint := "`zero_def` exposes `0` as `mk 0 1 _`; then it is the usual computation." },
  { name := "add_zero", dir := "Rat",
    hint := "Commute the sum, then use `MyRat.zero_add`." },
  { name := "neg_add_cancel", dir := "Rat",
    hint := "`neg_mk` computes the negation and `add_mk` the sum; `zero_def` exposes the right side." },
  { name := "neg_neg", dir := "Rat",
    hint := "`neg_mk` twice, then `mk_eq`." },
  { name := "mul_comm", dir := "Rat",
    hint := "As `add_comm`, with `mul_mk`." },
  { name := "mul_assoc", dir := "Rat",
    hint := "As `add_assoc`, with `mul_mk`." },
  { name := "one_mul", dir := "Rat",
    hint := "`one_def` exposes `1`; then `mul_mk` and `mk_eq`." },
  { name := "mul_one", dir := "Rat",
    hint := "Commute the product, then use `MyRat.one_mul`." },
  { name := "mul_zero", dir := "Rat",
    hint := "`zero_def`, `mul_mk`, `mk_eq`." },
  { name := "left_distrib", dir := "Rat",
    hint := "Compute the inner sum with `add_mk` before the outer product; on the right, compute the two products before their sum." },
  { name := "right_distrib", dir := "Rat",
    hint := "Commute the outer product and expand with `MyRat.left_distrib`. Then commute each remaining product with explicit arguments, such as `MyRat.mul_comm z x`." },
  { name := "mul_inv_cancel", dir := "Rat",
    hint := "After choosing a representative, get `ha : a ≠ 0` from `hx`: if `a = 0` then `x = 0` by `zero_def` and `mk_eq`. Then `inv_mk_of_ne hb ha`, `mul_mk`, `one_def`, `mk_eq`; the integer goal needs `Int.sign_mul_natAbs a` as a hypothesis." },
  -- RatOrder
  { name := "lt_irrefl", dir := "RatOrder",
    hint := "After a representative and `lt_mk`, the goal is `¬ a * b < a * b`." },
  { name := "lt_trans", dir := "RatOrder",
    hint := "With `lt_mk` in the hypotheses and goal: scale `hxy` by `f` and `hyz` by `b`, reorder products so the middle terms match, chain with `Int.lt_trans`, and cancel `d` with `Int.mul_lt_mul_right`." },
  { name := "lt_trichotomy", dir := "RatOrder",
    hint := "`simp only [lt_mk, mk_eq]` leaves a disjunction over ℤ comparing the same two products." },
  { name := "le_refl", dir := "RatOrder",
    hint := "After `le_mk`, the integer goal is reflexive." },
  { name := "le_trans", dir := "RatOrder",
    hint := "As `lt_trans`, with `Int.mul_le_mul_of_nonneg_right`, `Int.le_trans`, and `Int.mul_le_mul_right`." },
  { name := "add_lt_add_left", dir := "RatOrder",
    hint := "Scale `h` by `b * b`. Prove two `grind` equations that rewrite each side of the goal as a shared term plus the corresponding side of the scaled `h`; then `omega`." },
  { name := "add_le_add_left", dir := "RatOrder",
    hint := "As `add_lt_add_left`, with `Int.mul_le_mul_of_nonneg_right`." },
  { name := "mul_pos", dir := "RatOrder",
    hint := "`0 < mk a b hb` unfolds through `zero_def` and `lt_mk` to `0 * b < a * 1`, so `0 < a`. `Int.mul_pos` gives `0 < a * c`." },
  { name := "abs_nonneg", dir := "RatOrder",
    hint := "`abs_mk`, `zero_def`, `le_mk`; `omega` knows a cast `natAbs` is nonnegative." },
  { name := "abs_neg", dir := "RatOrder",
    hint := "`neg_mk`, `abs_mk`, `mk_eq`, then `Int.natAbs_neg`." },
  { name := "abs_mul", dir := "RatOrder",
    hint := "After `mk_eq`, rewrite with `Int.natAbs_mul`, then `push_cast` distributes the cast over the product." },
  { name := "abs_add_le", dir := "RatOrder",
    hint := "Reduce to `((a * d + c * b).natAbs : Int) ≤ (a.natAbs : Int) * d + (c.natAbs : Int) * b` and scale it by `b * d` with `Int.mul_le_mul_of_nonneg_right`.\nFor the reduction: start from `Int.natAbs_add_le (a * d) (c * b)`, rewrite with `Int.natAbs_mul`, transport with `exact_mod_cast`, `push_cast`, and replace `(b.natAbs : Int)` by `b` (`omega` proves that equation)." },
  { name := "abs_lt", dir := "RatOrder",
    hint := "After `simp only [abs_mk, neg_mk, lt_mk, Int.neg_mul]`, `by_cases h : 0 ≤ a`. In each branch, rewrite `(a.natAbs : Int) * d` as `a * d` or `-(a * d)`, record the sign of `a * d` (`Int.mul_nonneg` or `Int.mul_neg_of_neg_of_pos`), and `omega` decides the iff." },
  { name := "archimedean", dir := "RatOrder",
    hint := "Witness `a.natAbs + 1`. After `ofInt_def`, `lt_mk`, and `push_cast`, get `(↑a.natAbs + 1) * 1 ≤ (↑a.natAbs + 1) * b` from `Int.mul_le_mul_of_nonneg_left` and let `omega` assemble the chain." },
  { name := "exists_between", dir := "RatOrder",
    hint := "The midpoint `mk (a * d + c * b) (2 * (b * d)) _`. Each inequality is the hypothesis `a * d < c * b` scaled by `b` (respectively `d`), rearranged by `grind`-proved equations, and finished by `omega`." },
  -- Cauchy
  { name := "const_isCauchy", dir := "Cauchy",
    hint := "Any threshold works: `refine ⟨0, fun m n _ _ => ?_⟩`. The difference is `q - q`; `MyRat.sub_self` and `MyRat.abs_zero`." },
  { name := "add_isCauchy", dir := "Cauchy",
    hint := "`exists_half hε` gives `δ` with `δ + δ = ε`. Take thresholds for `f` and `g` at `δ` and use their `Nat.max`. Regroup the difference with `MyRat.add_sub_add`, then `abs_add_le` and `MyRat.add_lt_add` in a `calc`." },
  { name := "neg_isCauchy", dir := "Cauchy",
    hint := "Same threshold as `hf`. `MyRat.neg_sub_neg` turns `(-f m) - (-f n)` into `f n - f m`, and `abs_sub_comm` swaps it back." },
  { name := "equiv_refl", dir := "Cauchy",
    hint := "As `const_isCauchy`: `f n - f n = 0`." },
  { name := "equiv_symm", dir := "Cauchy",
    hint := "Reuse the threshold from `h`; `MyRat.abs_sub_comm`." },
  { name := "equiv_trans", dir := "Cauchy",
    hint := "Split `ε`, take the `Nat.max` of the two thresholds, and chain `MyRat.abs_sub_le (f n) (g n) (h n)` with `MyRat.add_lt_add`." },
  -- Real
  { name := "add_comm", dir := "Real",
    hint := "`induction x using MyReal.ind with | _ f hf =>` and likewise for `y`; `add_mk` on both sides; then `eq_of_equiv` and `equiv_of_eq` with `MyRat.add_comm` at each index." },
  { name := "add_assoc", dir := "Real",
    hint := "Three representatives, four `add_mk`s, pointwise `MyRat.add_assoc`." },
  { name := "zero_add", dir := "Real",
    hint := "`zero_def` exposes `0` as a constant sequence; `add_mk`; pointwise `MyRat.zero_add`." },
  { name := "add_zero", dir := "Real",
    hint := "`MyReal.add_comm`, then `MyReal.zero_add`." },
  { name := "neg_add_cancel", dir := "Real",
    hint := "`neg_mk`, `add_mk`, `zero_def`; pointwise `MyRat.neg_add_cancel`." },
  { name := "mul_comm", dir := "Real",
    hint := "As `add_comm`, with `mul_mk` and `MyRat.mul_comm`." },
  { name := "mul_assoc", dir := "Real",
    hint := "As `add_assoc`, with `mul_mk` and `MyRat.mul_assoc`." },
  { name := "one_mul", dir := "Real",
    hint := "`one_def`, `mul_mk`; pointwise `MyRat.one_mul`." },
  { name := "mul_one", dir := "Real",
    hint := "`MyReal.mul_comm`, then `MyReal.one_mul`." },
  { name := "left_distrib", dir := "Real",
    hint := "`add_mk`, three `mul_mk`s, `add_mk`; pointwise `MyRat.left_distrib`." },
  { name := "ofRat_add", dir := "Real",
    hint := "`ofRat_def` three times, then `add_mk`; the two sequences agree at every index by `rfl`." },
  { name := "ofRat_mul", dir := "Real",
    hint := "As `ofRat_add`, with `mul_mk`." },
  -- Capstone
  { name := "ofRat_pos", dir := "Capstone",
    hint := "`rw [lt_def, sub_zero, ofRat_def, isPos_mk]`. The eventual lower bound for the constant sequence at `q` is `q` itself, from index `0`." },
  { name := "cauchy_seq_converges", dir := "Capstone",
    hint := "Split `ε = δ + δ` and take `N` from `hf` at `δ`. For `k ≥ N`, unfold `<` (`show IsPos (ofRat ε - |ofRat (f k) - mk f hf|)`), compute a representative with `ofRat_def`, `sub_mk`, `abs_mk`, `sub_mk`, and `isPos_mk`, then offer the margin `δ` with threshold `N`.\nAt index `n ≥ N`, `|f k - f n| ≤ δ`; fold `a + -b` into `a - b` with `← MyRat.sub_eq`, and `MyRat.le_sub_iff` with `MyRat.add_le_add_left` finish." },
  -- Metric
  { name := "tendsto_const", dir := "Metric",
    hint := "Any threshold. `dist_self` makes the distance `0`, and `MyReal.ofRat_pos hε` is the bound." },
  { name := "converges_isCauchySeq", dir := "Metric",
    hint := "Split `ε` and take the threshold from `h` at the half. `dist_triangle` through `L`, `dist_comm` to orient the second distance, `MyReal.add_lt_add`, and `← MyReal.ofRat_add` to recombine." },
  -- RealField
  { name := "mul_inv_cancel", dir := "RealField",
    hint := "After a representative, `hnn : ¬ CauSeq.Null ⟨f, hf⟩` follows from `hx` and `mk_eq_zero_iff`, and `CauSeq.apart hnn` gives `q` and `N`. Rewrite with `inv_mk_of_not_null hnn`, `mul_mk`, `one_def`; `apply eq_of_equiv` and use threshold `N`: there `f n ≠ 0` (`MyRat.ne_zero_of_pos_abs`), so `MyRat.mul_inv_cancel` makes the difference `0`." },
  -- Complete
  { name := "approx", dir := "Complete",
    hint := "For `x = mk f hf`, `cauchy_seq_converges f hf ε hε` gives a threshold `N`; take `q := f N` and flip the difference with `MyReal.abs_sub_comm`." },
  { name := "approx_isCauchy", dir := "Complete",
    hint := "Give a quarter of `ε` to each approximation error and half to `|x m - x n|` (two uses of `exists_half`). Thresholds: `tolSeq_lt` for the quarter, `hx` for the half; use their `Nat.max`.\nMove the goal into ℝ with `← ofRat_lt_iff`, `ofRat_abs`, `ofRat_sub`, then chain `abs_sub_le` through `x m` and `x n`." },
  { name := "tendsto_of_approx", dir := "Complete",
    hint := "Half of `ε` for each piece. Thresholds: `tolSeq_lt` for the approximation error, `hq` for the limit. Chain `abs_sub_le` through `ofRat (q k)`; bound the first piece by `hqspec k` and `ofRat_lt`, the second by the bound from `hq`." },
  { name := "complete", dir := "Complete",
    hint := "Build `q` and `hqspec` with `Classical.choose` and `Classical.choose_spec` on `approx (x k) (tolSeq_pos k)`, as in the unit notes. Then `approx_isCauchy hx hqspec` shows `q` is Cauchy, the limit is `mk q _`, and `tendsto_of_approx` with `cauchy_seq_converges q _` finishes." }
]
...
private def analysisWelcome : String :=
  ...
  "Each world's unit notes (`lake exe leanlings guide`) introduce its idea and list\n" ++
  "the lemmas it needs; each exercise states the theorem and at most a sentence of\n" ++
  "strategy. Choosing the tools is the exercise. Use `#check` to read a lemma and\n" ++
  "`exact?` to search; `lake exe leanlings hint` gives a next step when you are stuck.\n" ++
  "Replace the `sorry` with a proof, then run `lake exe leanlings run` (or use watch mode).\n" ++
